            ...
            <div class="card-body table-responsive">
                <table class="table table-striped table-hover align-middle">
                    ...
                    <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($roomType->primary_image)
                                        <img src="{{ asset('storage/' . $roomType->primary_image) }}" width="70"
                                            height="55" class="rounded object-fit-cover">
                                    @else
                                ...
                                <td>
                                    ...
                                    <div class="small text-muted">{{ $roomType->category_label }} @if ($roomType->room_number)
                                            · #{{ $roomType->room_number }}
                                        @endif
                                    </div>
                                    <div class="small text-muted">
                                        {{ Str::limit($roomType->short_description ?? $roomType->description, 60) }}</div>
                                </td>
                                <td>
                                    Rs. {{ number_format($roomType->display_price, 2) }}
                                    @if ($roomType->discount_price)
                                        <div class="small text-muted"><s>Rs.
                                                {{ number_format($roomType->price_per_night, 2) }}</s></div>
                                    @endif
                                </td>
                                ...
                                <td>
                                    <span
                                        class="badge {{ $roomType->status === 'available' ? 'bg-success' : 'bg-danger' }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="text-center text-muted">No room types found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
